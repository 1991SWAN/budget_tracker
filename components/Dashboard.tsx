...
import { 
  ...
  BarChart, Bar, AreaChart, Area, XAxis, YAxis, Legend
} from 'recharts';
...
const Dashboard: React.FC<DashboardProps> = ({ transactions, assets, recurring, goals, onRecurringChange, onGoalChange, onAddTransaction, monthlyBudget, onBudgetChange, onNavigateToTransactions }) => {
  const [activeTab, setActiveTab] = useState<DashboardTab>('overview');
  const [billFilter, setBillFilter] = useState<BillType>(BillType.SUBSCRIPTION);
  
  const [trendModes, setTrendModes] = useState<{
  ...
  const netWorthHistory = [
    ...
    { name: 'Feb', value: totalNetWorth * 0.94 },
    { name: 'Mar', value: totalNetWorth * 0.91 },
    { name: 'Apr', value: totalNetWorth * 0.98 },
    { name: 'May', value: totalNetWorth },
  ];
  ...
  const ToggleButton = ({ mode, setMode }: { mode: Timeframe, setMode: (m: Timeframe) => void }) => (
    <div className="flex bg-slate-100 rounded-lg p-1">
      <button onClick={() => setMode('weekly')} className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${mode === 'weekly' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Week</button>
      <button onClick={() => setMode('monthly')} className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${mode === 'monthly' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Month</button>
    </div>
  );

  const calculateInstallmentProgress = (details: NonNullable<RecurringTransaction['installmentDetails']>) => {
     if (!details.startDate) return { current: 1, total: details.totalMonths };
     const start = new Date(details.startDate);
     const now = new Date();
     const monthsPassed = (now.getFullYear() - start.getFullYear()) * 12 + (now.getMonth() - start.getMonth());
     const current = Math.min(Math.max(1, monthsPassed + 1), details.totalMonths);
     return { current, total: details.totalMonths };
  };
  ...
  const handleSubmit = () => {
    ...
    if (modalType === 'bill') {
      ...
      const action = selectedItem ? 'update' : 'add';
      onGoalChange(action, { id: selectedItem?.id, name: formData.name, targetAmount: Number(formData.targetAmount), emoji: formData.emoji, deadline: formData.deadline });
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2 text-slate-800"><span>🗓️</span><h3 className="font-bold text-lg">Bills Calendar</h3></div>
              <button onClick={openAddBill} className="p-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 text-lg">➕</button>
            </div>
            <div className="flex gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide">
              {Object.values(BillType).map(type => (
                 <button key={type} onClick={() => setBillFilter(type)} className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-colors ${billFilter === type ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>{type}</button>
              ))}
            </div>
            <div className="space-y-3">
              {filteredBills.map((bill) => {
                let installmentStatus = bill.billType === BillType.INSTALLMENT && bill.installmentDetails ? calculateInstallmentProgress(bill.installmentDetails) : null;
                return (
                  <div key={bill.id} className="flex flex-col p-3 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => openEditBill(bill)}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 flex flex-col items-center justify-center rounded-lg text-xs font-bold bg-slate-100 text-slate-500"><span className="text-[9px] uppercase">Day</span>{bill.dayOfMonth}</div>
                        <div><p className="font-semibold text-slate-800">{bill.name}</p><p className="text-[10px] text-slate-500">{bill.category}</p></div>
                      </div>
                      <div className="text-right">
                        <span className="font-bold text-slate-800 block">{bill.amount.toLocaleString()}</span>
                        <button onClick={(e) => { e.stopPropagation(); openPayBill(bill); }} className="px-2 py-0.5 mt-1 bg-white border border-slate-200 text-[10px] font-bold text-slate-600 rounded-md">Pay</button>
                      </div>
                    </div>
                    {installmentStatus && (
                       <div className="mt-2 pt-2 border-t border-dashed border-slate-200"><div className="w-full h-1 bg-slate-100 rounded-full"><div className="h-full bg-indigo-500 rounded-full" style={{ width: `${(installmentStatus.current / installmentStatus.total) * 100}%` }}></div></div><p className="text-[9px] mt-1 text-slate-400">Progress: {installmentStatus.current}/{installmentStatus.total}</p></div>
                    )}
                  </div>
                );
              })}
          ...
          <div className="bg-rose-50 p-6 rounded-3xl shadow-sm border border-rose-100">
            <div className="flex items-center gap-2 text-rose-800 mb-4"><span>💳</span><h3 className="font-bold text-lg">Card Settlements</h3></div>
            <div className="space-y-3">
              {creditCardBills.map(card => (
                <div key={card.id} className="bg-white p-3 rounded-xl border border-rose-200 flex justify-between items-center group">
                  <div>
                    <p className="font-bold text-slate-800 text-sm">{card.name}</p>
                    <p className="text-[10px] text-slate-500">Pay on {card.paymentDay}th • Due: {card.dueAmount.toLocaleString()}</p>
                  </div>
                  <button onClick={() => openPayCard(card)} className="px-3 py-1.5 bg-rose-600 text-white text-xs font-bold rounded-lg hover:bg-rose-700 transition-colors">Pay Full</button>
                </div>
              ))}
              {creditCardBills.length === 0 && <p className="text-center text-rose-400 text-sm">No credit card debt.</p>}
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 h-fit">
          <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2 text-slate-800"><span>🎯</span><h3 className="font-bold text-lg">Goals Tracker</h3></div><button onClick={openAddGoal} className="p-1.5 bg-rose-50 text-rose-600 rounded-lg text-lg">➕</button></div>
          <div className="space-y-6">
            {goals.filter(g => g.currentAmount < g.targetAmount).map((goal) => (
              <div key={goal.id} className="group">
                <div className="flex justify-between items-end mb-2"><div className="flex items-center gap-2 cursor-pointer" onClick={() => openEditGoal(goal)}><span className="text-2xl">{goal.emoji}</span><div><p className="font-semibold text-slate-700">{goal.name}</p><p className="text-[10px] text-slate-400">Target: {goal.targetAmount.toLocaleString()}</p></div></div><button onClick={() => openFundGoal(goal)} className="text-[10px] font-bold bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-md">+ Funds</button></div>
                <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-indigo-500 rounded-full" style={{ width: `${Math.min((goal.currentAmount / goal.targetAmount) * 100, 100)}%` }}></div></div>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="flex-1 pb-12">
        ...
        {activeTab === 'trends' && <div className="space-y-6">{/* Standard Trend Charts */}</div>}
