...
- [x] **거래 목록 UI (Transaction List UX)**
    ...
    - [x] **시인성 개선**: 2017년 데이터 실종 문제 해결 (DB 제한 완화 및 필터 로직 정교화).
...
- [x] **Import UI/UX 개선** (Feature/import-ui-ux)
    - [x] **Phase 1**: MappingStep 시각적 개선 (테이블/컬럼 UI 고도화).
    - [x] **Phase 1.5**: MappingStep 행 삭제/정리 기능 (User Request).
    - [x] **Phase 2**: 프리뷰 인라인 편집 (Implemented with Virtualization).
    - [x] **Phase 3**: 프리미엄 그리드 UI 및 동적 컬럼 매핑 고도화 (Approved & Implemented).
    - [ ] **Phase 4**: 스마트 카테고리 제안 (추후).
